...
const CalculationsTable: React.FC<Props> = ({ data, columns }) => {
  ...
  const nonDivisionTable = useAppSelector(
    (store) => store.baseballHomeNonDivision2017
  );
  const divisionTable = useAppSelector(
  ...
  useEffect(() => {
    const getData = async () => {
      const { data } = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/draft-kings/test-get-data`
      );
      const calculatedArr = data.map((obj: FullCalculationTable) => {
        ...
        const trueOverProbability = calculateTrueOverProbability(
          obj,
          nonDivisionTable,
          divisionTable
        );
        const trueUnderProbability =
          calculateTrueUnderProbability(trueOverProbability);
    ...
    <div className="my-5 flex flex-col items-center">
      <div className="w-full">
        <div className="w-full overflow-x-auto block">
          <table
            {...getTableProps()}
            className="inline-block overflow-x-auto table-fixed"
          >
