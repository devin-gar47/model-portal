...
function calculateTrueOverProbability(
  ...
  const nonDivRowIndex = nonDivisionTableInfo.findIndex(
    (row: any) => row.ou === ou
  );
  ...
  if (divisionGame === "YES") {
    if (pick === "FAV") {
      if (line === "1.5") {
        if (g1 === "YES")
          division_g1RecordInfo = divRowObj.g1_fav_o1point5OR3point5;
      ...
      if (homeML < -200) {
        if (line === "2.5")
          division_homeMLRecordInfo = divRowObj.home_mlo2point5;
        else if (line === "3.5")
          division_homeMLRecordInfo = divRowObj.home_mlo3point5;
      }
    } else {
      if (line === "1.5") {
        if (g1 === "YES")
          division_g1RecordInfo = divRowObj.g1_dog_o1point5OR3point5;
      ...
      if (roadML < -200) {
        if (line === "1.5")
          division_roadMLRecordInfo = divRowObj.ifRoadMLOnePointFive;
      }
    }
  }
...
function calculateSuggestion(
  ...
  if (trueOverProbability > impliedOverProbability)
    return `${(
      parseFloat(trueOverProbability) - parseFloat(impliedOverProbability)
    ).toFixed(2)}% Over`;
  return `${(
    parseFloat(trueUnderProbability) - parseFloat(impliedUnderProbability)
  ).toFixed(2)}% Under`;
}
